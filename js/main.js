...
function isHTML5Supported() {
    if (window.location.href.indexOf("noHTML5") != -1) return false;
    return window.HTMLCanvasElement;
}

// use 768 images for some config
function wantsHighRes() {
    if (/MSIE (\d+\.\d+);/.test(navigator.userAgent)) return true; // Accelerated IE?
    if (/CrOS /.test(navigator.userAgent)) return false; // Chromebook
...
function resizeElements() {
    ...
    if (/Chrome/.test(navigator.userAgent)) aGameDiv.style.overflow = "visible";
...
function startup() {
    ...
    if (aUserId != null) gApp.SetUserId(aUserId);
    ...
    // webgl or not
    if (window.location.href.indexOf("nogl") != -1) gApp.SetUseGL(false);
    else gApp.SetUseGL(true);

    // save app reference to curApp
    if (typeof curApp == "undefined") curApp = gApp;

    // html5 or not
    if (isHTML5Supported()) {
        JSFExt_Init(gApp, document.getElementById("GameCanvas"));
    } else {
        if (window["JSFExt_MinInit"]) window["JSFExt_MinInit"](gApp);
    }

    // high resolution or not
    if (!wantsHighRes()) gApp.SetArtRes(480);

    // if not webgl, access different set of resources
    if (!gApp.GetUseGL()) gApp.SetPathPrefix("html5canvas/");
