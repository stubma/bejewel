...
GameFramework.resources.PIInterpolator.prototype = {
    ...
    GetValueAt: function GameFramework_resources_PIInterpolator$GetValueAt(theTime) {
        ...
        var aScaledTime =
            this.mInterpolatorPointVector[0].mTime +
            theTime *
                (this.mInterpolatorPointVector[this.mInterpolatorPointVector.length - 1].mTime -
                    this.mInterpolatorPointVector[0].mTime);
        for (var i = 1; i < (this.mInterpolatorPointVector.length | 0); i++) {
            ...
            if (aScaledTime >= aP1.mTime && aScaledTime <= aP2.mTime) {
                return GameFramework.Utils.LerpColor(
                    aP1.mValue,
                    aP2.mValue,
                    Math.min(1.0, (aScaledTime - aP1.mTime) / (aP2.mTime - aP1.mTime))
                );
            }
...
JSFExt_AddInitFunc(function () {
    GameFramework.resources.PIInterpolator.registerClass(
        "GameFramework.resources.PIInterpolator",
        null,
