...
GameFramework.widgets.EditWidget.prototype = {
    ...
    Draw: function GameFramework_widgets_EditWidget$Draw(g) {
        ...
        for (var aPass = 0; aPass < 2; aPass++) {
            ...
            for (var i = this.mLeftPos; i < this.mString.length; i++) {
                ...
                if (
                    (this.mHilitePos != -1 && i >= this.mHilitePos && i < this.mCursorPos) ||
                    (i >= this.mCursorPos && i < this.mHilitePos)
                ) {
                    if (aPass == 0) {
                        ...
                        try {
                            this.mFont.Draw(
                                g,
                                "" + String.fromCharCode(aChar),
                                this.applyJustification(aX, strWidth),
                                (this.mHeight - this.mFont.GetHeight()) / 2 + this.mFont.GetAscent()
                            );
                        } finally {
                            _t4.Dispose();
                        }
    ...
    IsPartOfWord: function GameFramework_widgets_EditWidget$IsPartOfWord(theChar) {
        return (
            (theChar >= 65 && theChar <= 90) ||
            (theChar >= 97 && theChar <= 122) ||
            (theChar >= 48 && theChar <= 57) ||
            (theChar >= 0xbf && theChar <= 0x2c7) ||
            theChar == 95
        );
    },
    ProcessKey: function GameFramework_widgets_EditWidget$ProcessKey(theKey, theChar) {
        ...
        if (
            theKey == GameFramework.KeyCode.Shift ||
            theKey == GameFramework.KeyCode.Control ||
            theKey == GameFramework.KeyCode.Command
        ) {
            return;
        }
        var bigChange = false;
        ...
        if (theKey == GameFramework.KeyCode.Left) {
            ...
            if (controlDown) {
                while (
                    this.mCursorPos < (this.mString.length | 0) - 1 &&
                    this.IsPartOfWord(this.mString.charCodeAt(this.mCursorPos + 1))
                ) {
                    this.mCursorPos++;
                }
                while (
            ...
            if (this.mString.length > 0) {
                if (this.mHilitePos != -1 && this.mHilitePos != this.mCursorPos) {
                    this.mString =
                        this.mString.substr(0, Math.min(this.mCursorPos, this.mHilitePos) | 0) +
                        this.mString.substr(Math.max(this.mCursorPos, this.mHilitePos) | 0);
                    this.mCursorPos = Math.min(this.mCursorPos, this.mHilitePos) | 0;
                    this.mHilitePos = -1;
                    bigChange = true;
                } else {
                    if (this.mCursorPos > 0) {
                        this.mString =
                            this.mString.substr(0, this.mCursorPos - 1) + this.mString.substr(this.mCursorPos);
                    } else {
            ...
            if (this.mString.length > 0) {
                if (this.mHilitePos != -1 && this.mHilitePos != this.mCursorPos) {
                    ...
                    if (this.mCursorPos < (this.mString.length | 0)) {
                        this.mString =
                            this.mString.substr(0, this.mCursorPos) + this.mString.substr(this.mCursorPos + 1);
                    }
                    if (this.mCursorPos != this.mLastModifyIdx) {
                        bigChange = true;
                    }
            ...
            if (
                uTheChar >= 32 &&
                uTheChar <= range &&
                this.mFont.StringWidth(aString) > 0 &&
                this.mValidator.EditWidgetAllowChar(this, theChar)
            ) {
                if (this.mHilitePos != -1 && this.mHilitePos != this.mCursorPos) {
                    this.mString =
                        this.mString.substr(0, Math.min(this.mCursorPos, this.mHilitePos) | 0) +
                        String.fromCharCode(theChar) +
                        this.mString.substr(Math.max(this.mCursorPos, this.mHilitePos) | 0);
                    this.mCursorPos = Math.min(this.mCursorPos, this.mHilitePos) | 0;
                    this.mHilitePos = -1;
                    bigChange = true;
                    ...
                    this.mString =
                        this.mString.substr(0, this.mCursorPos) +
                        String.fromCharCode(theChar) +
                        this.mString.substr(this.mCursorPos);
    ...
    FocusCursor: function GameFramework_widgets_EditWidget$FocusCursor(bigJump) {
        ...
        if (this.mFont != null) {
            ...
            while (
                this.mWidth - 8 > 0 &&
                this.mFont.StringWidth(aString.substr(0, this.mCursorPos)) -
                    this.mFont.StringWidth(aString.substr(0, this.mLeftPos)) >=
                    this.mWidth - 8
            ) {
                if (bigJump) {
    ...
    HiliteWord: function GameFramework_widgets_EditWidget$HiliteWord() {
        ...
        if (this.mCursorPos < (aString.length | 0)) {
            ...
            while (
                this.mCursorPos < (aString.length | 0) - 1 &&
                this.IsPartOfWord(aString.charCodeAt(this.mCursorPos + 1))
            ) {
                this.mCursorPos++;
            }
            if (this.mCursorPos < (aString.length | 0)) {
...
JSFExt_AddInitFunc(function () {
    GameFramework.widgets.EditWidget.registerClass(
        "GameFramework.widgets.EditWidget",
