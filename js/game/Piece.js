...
Game.Piece.GetAngleRadiusColorFrame = function Game_Piece$GetAngleRadiusColorFrame(theAngle, theColor, theFrame) {
    ...
    if (anIdx < 0) {
        ...
        var aPct = aNum - anIdx;
        return (
            Game.Piece.GetRadiusPoint(theFrame, aColorIdx, anIdx) * (1.0 - aPct) +
            Game.Piece.GetRadiusPoint(theFrame, aColorIdx, anIdx + 1) * aPct
        );
    }
};
        ...
        if (this.IsFlagSet(Game.Piece.EFlag.FLAME)) {
            for (var i = 0; i < 4; i++) {
                ...
                if (
                    ...
                    GameFramework.Utils.GetRandFloatU() > 0.4 + Game.BejApp.mBejApp.mFXScale * 0.6
                ) {
                    continue;
                }
                ...
                if (isFlame) {
                    ...
                    var doRedFlame =
                        this.mBoard.mWantsReddishFlamegems &&
                        Game.Util.Rand() % 4 <= 0 &&
                        aManager == this.mBoard.mPreFXManager;
                    ...
                    anEffect.mColor = GameFramework.gfx.Color.RGBToInt(
                        128,
                        (Game.Util.Rand() % 32) + 48,
                        (Game.Util.Rand() % 24) + 24
                    );
                    var doRedFlame_2 =
                        this.mBoard.mWantsReddishFlamegems &&
                        Game.Util.Rand() % 3 <= 0 &&
