Game.HyperspaceUltra = function Game_HyperspaceUltra(theBoard) {
    ...
    this.mAnimSeq.Reset();
    this.mCameraPersp.Init(
        this.GetFocalLength(),
        GameFramework.BaseApp.mApp.mWidth / GameFramework.BaseApp.mApp.mHeight,
        100.0,
        this.GetZFarClip()
    );
    ...
    for (var aRow = 0; aRow < Game.Board.NUM_ROWS; aRow++) {
        for (var aCol = 0; aCol < Game.Board.NUM_COLS; aCol++) {
            ...
            this.mGemInfo[this.mGemInfo.mIdxMult0 * aRow + aCol].Init(
                theBoard.mBoard[theBoard.mBoard.mIdxMult0 * aRow + aCol],
                this.mAnimSeq
            );
        }
    }
...
Game.HyperspaceUltra.prototype = {
    ...
    SetState: function Game_HyperspaceUltra$SetState(state) {
        ...
        switch (this.mState) {
            ...
            case Game.HyperspaceUltra.HyperSpaceState.GemRise: {
                ...
                var mapBgIdToHyperspaceTubeTextures = Array.Create(
                    4,
                    4,
    ...
    Update3DGems: function Game_HyperspaceUltra$Update3DGems() {
        if (this.mAnimSeq.GetCurFrame() >= 68) {
            for (var row = 0; row < Game.Board.NUM_ROWS; ++row) {
                for (var col = 0; col < Game.Board.NUM_COLS; ++col) {
                    this.mGemInfo[this.mGemInfo.mIdxMult0 * row + col].mPiece =
                        this.mBoardInfo.mBoard.mBoard[this.mBoardInfo.mBoard.mBoard.mIdxMult0 * row + col];
                }
            }
        }
        ...
        for (var row_2 = 0; row_2 < Game.Board.NUM_ROWS; ++row_2) {
            for (var col_2 = 0; col_2 < Game.Board.NUM_COLS; ++col_2) {
                ...
                if (
                    this.mState == Game.HyperspaceUltra.HyperSpaceState.LandOnBoard &&
                    gemInfoPointer.mDraw3D &&
                    this.mAnimSeq.GetCurFrame() >= gemInfoPointer.mBoardHitFrame
                ) {
                    gemInfoPointer.mPiece.mAlpha.SetConstant(1.0);
                    gemInfoPointer.mDraw3D = false;
                    var mapHitCountToSound = Array.Create(
                        7,
                        7,
                        Game.Resources["SOUND_HYPERSPACE_GEM_LAND_1"],
                        ...
                        Game.Resources["SOUND_HYPERSPACE_GEM_LAND_7"]
                    );
                    var landingFx;
                    var landingSoundFx;
        ...
        if (
            this.mState < Game.HyperspaceUltra.HyperSpaceState.LandOnBoard &&
            this.mState >= Game.HyperspaceUltra.HyperSpaceState.BoardShatter
        ) {
    ...
    Draw3DWarpTubeTube: function Game_HyperspaceUltra$Draw3DWarpTubeTube(g) {
        ...
        Game.HyperspaceUltra.mG3D.SetBlend(
            GameFramework.gfx.Graphics3D.EBlend.Default,
            GameFramework.gfx.Graphics3D.EBlend.Default
        );
        ...
        try {
            ...
            var streamFx = 0.5;
            var brightness = 1.0;
            var alphaVals = Array.Create(
                4,
                4,
                this.mMinAlpha.get_v(),
                this.mWarpTubeTextureFade.get_v(),
                streamFx,
                brightness
            );
    ...
    Draw3DWarpTubeCap: function Game_HyperspaceUltra$Draw3DWarpTubeCap(g) {
        if (
            this.mState != Game.HyperspaceUltra.HyperSpaceState.PortalRide &&
            this.mState != Game.HyperspaceUltra.HyperSpaceState.LandOnBoard
        ) {
            return;
        }
        ...
        Game.HyperspaceUltra.mG3D.SetBlend(
            GameFramework.gfx.Graphics3D.EBlend.SrcColor,
            GameFramework.gfx.Graphics3D.EBlend.One
        );
        Game.HyperspaceUltra.mG3D.SetTextureWrap(0, true);
    ...
    Draw2DBoardSmash: function Game_HyperspaceUltra$Draw2DBoardSmash(g) {
        ...
        try {
            var _t7 = g.PushScale(
                this.mShatterScale.get_v(),
                this.mShatterScale.get_v(),
                shaterAnimWidth / 2.0,
                shaterAnimHeight / 2.0
            );
            try {
    ...
    Draw3DGemsEveryOther: function Game_HyperspaceUltra$Draw3DGemsEveryOther(g) {
        ...
        for (var $enum9 = 0; $enum9 < $srcArray9.length; $enum9++) {
            ...
            if (
                colorIndex >= 0 &&
                colorIndex < (Game.DM.EGemColor.HYPERCUBE | 0) &&
                gemInfoPointer.mPiece.mRow % 2 == 0
            ) {
            }
        }
    },
    Draw3DGems: function Game_HyperspaceUltra$Draw3DGems(g) {
        ...
        try {
            ...
            cameraPoistion = new GameFramework.geom.Vector3(
                cameraPoistion.x,
                cameraPoistion.y,
                cameraPoistion.z * -1.0
            );
                ...
                for (var $enum11 = 0; $enum11 < $srcArray11.length; $enum11++) {
                    ...
                    if (gemInfoPointer.mDraw3D) {
                        ...
                        if (colorIndex < (Game.DM.EGemColor.HYPERCUBE | 0)) {
                            ...
                            coords.GetOutboundMatrix(matWorld);
                            Game.HyperspaceUltra.mG3D.SetWorldTransform(matWorld);
                            var lightPosition = gemInfoPointer.mCoords.t.Add(
                                Game.HyperspaceUltra.mapColorIndexToLightOffset[colorIndex]
                            );
                            renderEffect.SetVector("lightPosition", lightPosition);
                            renderEffect.SetFloatArray(
                                "ambientMaterialColor",
                                Game.HyperspaceUltra.mapColorIndexToMaterial[colorIndex].ambient
                            );
                            renderEffect.SetFloatArray(
                                "diffuseMaterialColor",
                                Game.HyperspaceUltra.mapColorIndexToMaterial[colorIndex].diffuse
                            );
                            renderEffect.SetFloatArray(
                                "specularMaterialColor",
                                Game.HyperspaceUltra.mapColorIndexToMaterial[colorIndex].specular
                            );
                            renderEffect.SetFloat(
                                "specularPower",
                                Game.HyperspaceUltra.mapColorIndexToMaterial[colorIndex].power
                            );
                ...
                for (var $enum12 = 0; $enum12 < $srcArray12.length; $enum12++) {
                    ...
                    if (gemInfoPointer_2.mDraw3D) {
                        ...
                        if (colorIndex_2 < (Game.DM.EGemColor.HYPERCUBE | 0)) {
                            ...
                            var distToCameraScale_2 = Math.max(
                                Math.min(gemInfoPointer_2.mDistToCamera / 3425.0, 1.0),
                                0.5
                            );
                        ...
                        if (colorIndex_2 == (Game.DM.EGemColor.HYPERCUBE | 0) || aPiece.mBoundEffects.length > 0) {
                            ...
                            var _t13 = g.PushTranslate(
                                160 + gemInfoPointer_2.mPosScreen.x * 800 + 800,
                                gemInfoPointer_2.mPosScreen.y * -600 + 600
                            );
                            try {
                                if (hasPreFX || colorIndex_2 == (Game.DM.EGemColor.HYPERCUBE | 0)) {
                                    aRenderEffectPass.Dispose();
                                    Game.HyperspaceUltra.mG3D.Setup2DDrawing(
                                        Math.max(0.0, gemInfoPointer_2.mPosScreen.z + 0.0025)
                                    );
                                    Game.HyperspaceUltra.mG3D.SetDepthState(
                                        GameFramework.gfx.Graphics3D.ECompareFunc.Less,
                                        false
                                    );
                                    ...
                                    if (colorIndex_2 == (Game.DM.EGemColor.HYPERCUBE | 0)) {
                                        ...
                                        try {
                                            this.mBoardInfo.mBoard.DrawPiece(
                                                g,
                                                aPiece,
                                                gemInfoPointer_2.mScaleScreen,
                                                true
                                            );
                                        } finally {
                                            _t14.Dispose();
    ...
    DrawBillboardEffects: function Game_HyperspaceUltra$DrawBillboardEffects(g, gemInfoPointer, front) {
        ...
        Game.HyperspaceUltra.mG3D.SetBackfaceCulling(false, false);
        var _t19 = g.PushTranslate(
            160 + gemInfoPointer.mPosScreen.x * xScreenCenter + xScreenCenter,
            gemInfoPointer.mPosScreen.y * -yScreenCenter + yScreenCenter
        );
        try {
            var didSetup = false;
            ...
            for (var anEffectIdx = 0; anEffectIdx < (aPiece.mBoundEffects.length | 0); anEffectIdx++) {
                var anEffect = aPiece.mBoundEffects[anEffectIdx];
                if (
                    (anEffect.mFXManager == this.mBoardInfo.mBoard.mPreFXManager && !front) ||
                    (anEffect.mFXManager == this.mBoardInfo.mBoard.mPostFXManager && front)
                ) {
                    if (!didSetup) {
                        Game.HyperspaceUltra.mG3D.Setup2DDrawing(gemInfoPointer.mPosScreen.z);
                        Game.HyperspaceUltra.mG3D.SetDepthState(GameFramework.gfx.Graphics3D.ECompareFunc.Less, false);
...
Game.HyperspaceUltra.staticInit = function Game_HyperspaceUltra$staticInit() {
    ...
    Game.HyperspaceUltra.mapColorIndexToMaterial = Array.Create(
        7,
        7,
        new Game.HyperMaterial(
            Array.Create(4, null, 0.6, 0.6, 0.6, 0.6),
            Array.Create(4, null, 1.0, 1.0, 1.0, 1.0),
            Array.Create(4, null, 1.0, 1.0, 1.0, 0.6),
            55.0
        ),
        new Game.HyperMaterial(
            Array.Create(4, null, 0.7, 0.7, 0.7, 0.7),
            Array.Create(4, null, 1.0, 1.0, 1.0, 1.0),
            Array.Create(4, null, 1.0, 1.0, 1.0, 0.6),
            55.0
        ),
    ...
    Game.HyperspaceUltra.mapColorIndexToLightOffset = Array.Create(
        7,
        7,
        new GameFramework.geom.Vector3(50.0, 150.0, -200.0),
        new GameFramework.geom.Vector3(50.0, 150.0, -200.0),
        new GameFramework.geom.Vector3(50.0, 150.0, -200.0),
        new GameFramework.geom.Vector3(-100.0, -100.0, -200.0),
        new GameFramework.geom.Vector3(100.0, 50.0, 0.0),
        new GameFramework.geom.Vector3(50.0, 150.0, -200.0),
        new GameFramework.geom.Vector3(-100.0, 200.0, 0.0)
    );
