...
Game.Background.StartBkgLoad = function Game_Background$StartBkgLoad(theBkgIdx) {
    if (!Game.Background.mIsBkgLoaded[theBkgIdx]) {
        ...
        if (Game.Background.mBkgGroupNames[theBkgIdx] != null) {
            aResourceStreamer = GameFramework.BaseApp.mApp.mResourceManager.StreamResourceGroup(
                Game.Background.mBkgGroupNames[theBkgIdx]
            );
        } else {
            aResourceStreamer = GameFramework.BaseApp.mApp.mResourceManager.StreamImageFromPath(
                Game.Background.mFlattenedNames[theBkgIdx]
            );
        }
...
Game.Background.prototype = {
    ...
    TryLoadBackground: function Game_Background$TryLoadBackground(theIdx) {
        if (Game.Background.mIsBkgLoaded[theIdx]) {
            ...
            if (Game.Background.mLoadedImages[theIdx] != null) {
                ...
                this.mImage = GameFramework.BaseApp.mApp.mResourceManager.GetImageResourceById(
                    Game.Background.mFlattenedNames[theIdx]
                );
                if (GameFramework.BaseApp.mApp.get_Is3D() && !Game.BejApp.mBejApp.mIsSlow) {
                    this.mAnim = GameFramework.BaseApp.mApp.mResourceManager.GetPopAnimResourceById(
                        Game.Background.mPopAnimNames[theIdx]
                    );
    ...
    NextBkg: function Game_Background$NextBkg() {
        ...
        for (var anIdx = 0; anIdx < Game.Background.mBkgGroupNames.length; anIdx++) {
            if (
                anIdx != 0 &&
                anIdx != aPrevBkgIdx &&
                anIdx != this.mCurBkgIdx &&
                anIdx != aLoadingBkgIdx &&
                Game.Background.mIsBkgLoaded[anIdx]
            ) {
    ...
    Update: function Game_Background$Update() {
        ...
        if (
            ...
            Game.BejApp.mBejApp.mBoard.mHyperspace == null &&
            this.mWantAnim &&
            Game.BejApp.mBejApp.mBoard.mSideXOff.get_v() == 0
        ) {
        ...
        if (
            Game.BejApp.mBejApp.get_Is3D() &&
            true &&
            this.mWantAnim &&
            this.mStage == 1 &&
            (this.mVisible || this.mParent == null)
        ) {
            this.mStage++;
        }
        {
            ...
            if (
                !this.mHasRenderTargetFlatImage &&
                this.mAnim != null &&
                !this.mKeepFlatImage &&
                this.mImageOverlayAlpha.GetOutVal() == 0.0
            ) {
            }
        }
        if (this.mAnimActive) {
