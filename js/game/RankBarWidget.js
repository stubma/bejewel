Game.RankBarWidget = function Game_RankBarWidget(theWidth, theBoard, theRankUpDialog, drawText) {
    ...
    if (this.mBoard != null) {
        ...
        app.mProfile.mOfflineRankPoints +=
            (this.mBoard.mGameStats[Game.DM.EStat.POINTS | 0] / this.mBoard.GetRankPointMultiplier() / 1000) | 0;
...
Game.RankBarWidget.prototype = {
    ...
    Update: function Game_RankBarWidget$Update() {
        ...
        if (this.mRankDelay > 0) {
            ...
            this.mDispRankPointsOffset = Math.min(
                0.0,
                this.mDispRankPointsOffset + -this.mDispRankPointsOffset / 100.0 + 0.1
            );
            var aNewRank =
                app.mProfile.GetRankAtPointsK(
                    (this.mDispRankPointsOffset | 0) + Game.BejApp.mBejApp.mProfile.mOfflineRankPoints
                ) | 0;
    ...
    Draw: function Game_RankBarWidget$Draw(g) {
        ...
        var aNextRankPct = Math.min(
            (aRankPoints - aBaseRankPoints + this.mDispRankPointsOffset) / (aNextRankPoints - aBaseRankPoints),
            1.0
        );
        g.DrawImage(Game.Resources["IMAGE_RECORDS_RANK_PLATE"].get_OffsetImage(), 0, 0);
        var aShowPct = aNextRankPct;
        if (aShowPct != 0.0) {
            var anImageInst = Game.Resources["IMAGE_RECORDS_RANK_FILL"].CreateImageInstRect(
                0,
                0,
                (Game.Resources["IMAGE_RECORDS_RANK_FILL"].mPhysCelWidth * aShowPct) | 0,
                Game.Resources["IMAGE_RECORDS_RANK_FILL"].mPhysCelHeight
            );
            g.DrawImage(
                anImageInst,
                Game.Resources["IMAGE_RECORDS_RANK_FILL"].mOffsetX,
                Game.Resources["IMAGE_RECORDS_RANK_FILL"].mOffsetY
            );
        }
        ...
        if (this.mDrawText) {
            ...
            if (this.mBoard != null) {
                g.DrawStringEx(
                    app.mRankNames[Math.min(aRank | 0, (app.mRankNames.length | 0) - 1) | 0],
                    this.mWidth / 2 + 40,
                    aY,
                    -1,
                    0
                ...
                g.DrawStringEx(
                    String.format("{0}k to go", GameFramework.Utils.CommaSeperate(this.GetRankPointsRemaining() | 0)),
                    this.mWidth - 40,
                    aY,
                    -1,
                    1
    ...
    GetRankPointsRemaining: function Game_RankBarWidget$GetRankPointsRemaining() {
        return (
            Math.max(
                0,
                ((this.GetNextRankPoints() - this.GetRankPoints() - this.mDispRankPointsOffset) *
                    this.mBoard.GetRankPointMultiplier()) |
                    0
            ) | 0
        );
    },
            ...
            switch (theChar) {
                ...
                case 43: {
                    ...
                    Game.BejApp.mBejApp.mProfile.mOfflineRankPoints =
                        (Game.BejApp.mBejApp.mProfile.GetRankPointsK(Game.BejApp.mBejApp.mProfile.mOfflineRank | 0) |
                            0) +
                        50;
