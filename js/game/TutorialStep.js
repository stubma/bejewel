...
Game.TutorialStep.AddGemGridXYToRect = function Game_TutorialStep$AddGemGridXYToRect(
    theBoard,
    theRect,
    theCol,
    theRow,
    theHighlightRectGemPadding
) {
    if (theHighlightRectGemPadding === undefined) {
        theHighlightRectGemPadding = 0.0;
    }
    var gemRect = new GameFramework.TRect(
        theBoard.GetColScreenX(theCol) - theHighlightRectGemPadding,
        theBoard.GetRowScreenY(theRow) - theHighlightRectGemPadding,
        Game.Board.GEM_WIDTH + theHighlightRectGemPadding * 2.0,
        Game.Board.GEM_HEIGHT + theHighlightRectGemPadding * 2.0
    );
...
Game.TutorialStep.prototype = {
    ...
    Finish: function Game_TutorialStep$Finish() {
        if (
            this.mSpecialBehavior == Game.TutorialStep.ESpecialBehavior.TimeGem &&
            ...
            if (this.mHighlightRect != null) {
                if (this.mSpecialBehavior == Game.TutorialStep.ESpecialBehavior.HintBtn) {
                    ...
                    if (this.mSpecialBehavior == Game.TutorialStep.ESpecialBehavior.MultiplierUp) {
                        var sepX = 450.0;
                        Game.TutorialMgr.DrawHighlightBox(
                            g,
                            new GameFramework.TRect(62.0, 240.0, 360.0, 500.0),
                            new GameFramework.TRect(0, 0, sepX, GameFramework.BaseApp.mApp.mHeight)
                        );
                        Game.TutorialMgr.DrawHighlightBox(
                            g,
                            this.mHighlightRect,
                            new GameFramework.TRect(
                                sepX,
                                0,
                                GameFramework.BaseApp.mApp.mWidth - sepX,
                                GameFramework.BaseApp.mApp.mHeight
                            )
                        );
                    } else if (
                        this.mSpecialBehavior == Game.TutorialStep.ESpecialBehavior.TimeGem &&
                        ...
                        Game.TutorialMgr.DrawHighlightBox(
                            g,
                            new GameFramework.TRect(62.0, 240.0, 360.0, 500.0),
                            new GameFramework.TRect(0, 0, sepX_2, GameFramework.BaseApp.mApp.mHeight)
                        );
                        Game.TutorialMgr.DrawHighlightBox(
                            g,
                            this.mHighlightRect,
                            new GameFramework.TRect(
                                sepX_2,
                                0,
                                GameFramework.BaseApp.mApp.mWidth - sepX_2,
                                GameFramework.BaseApp.mApp.mHeight
                            )
                        );
                    } else {
    ...
    Start: function Game_TutorialStep$Start() {
        ...
        if (this.mType == Game.TutorialStep.EType.ModalDialogOkBtnClear) {
            ...
            this.mType == Game.TutorialStep.EType.ModalDialogMoveClear ||
            this.mType == Game.TutorialStep.EType.ModalDialog
        ) {
            this.mHintDlg = new Game.HintDialog(this.mTextHeader, this.mText, true, false);
            this.mHintDlg.mWantsDarken = false;
        ...
        this.mHintDlg.AddEventListener(
        ...
        if (this.mDialogWidth != 0.0 || this.mDialogHeight != 0.0) {
            ...
            this.mHintDlg.Resize(
                this.mHintDlg.mX,
                this.mHintDlg.mY,
                this.mDialogWidth != 0.0 ? this.mDialogWidth : this.mHintDlg.mWidth,
                this.mDialogHeight != 0.0 ? this.mDialogHeight : this.mHintDlg.mHeight
            );
    ...
    PointArrowAt: function Game_TutorialStep$PointArrowAt(theBoard, theCol, theRow, theArrowDir) {
        var gemRect = new GameFramework.TRect(
            theBoard.GetColScreenX(theCol),
            theBoard.GetRowScreenY(theRow),
    ...
    AddGemGridXY: function Game_TutorialStep$AddGemGridXY(
        theBoard,
        theCol,
        theRow,
        theIsUISelectable,
        theAddToHighlightRect
    ) {
        ...
        if (theAddToHighlightRect) {
            this.mHighlightRect = Game.TutorialStep.AddGemGridXYToRect(
                theBoard,
                this.mHighlightRect,
                theCol,
                theRow,
                this.mHighlightRectGemPadding
            );
        }
    },
};
