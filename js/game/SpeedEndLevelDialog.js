...
Game.SpeedEndLevelDialog.prototype = {
    ...
    Draw: function Game_SpeedEndLevelDialog$Draw(g) {
        ...
        g.DrawString(
            String.format("TIME +{0}:{1:00}", (this.mTotalBonusTime / 60) | 0, this.mTotalBonusTime % 60),
            830,
            723
        );
        ...
        for (var aPointSection_2 = 0; aPointSection_2 < (this.mPointsBreakdown.length | 0); aPointSection_2++) {
            for (var aPointTypeIdx_2 = aPointTypes.length - 1; aPointTypeIdx_2 >= 0; aPointTypeIdx_2--) {
                ...
                var aBarImages = Array.Create(
                    3,
                    3,
                    Game.Resources["IMAGE_GAMEOVER_BAR__PINK"],
                    Game.Resources["IMAGE_GAMEOVER_BAR_ORANGE"],
                    Game.Resources["IMAGE_GAMEOVER_BAR_YELLOW"]
                );
                var aHeight =
                    (((225.0 * aCurSectionPoints_2) / (aChunkPoints * 5.0)) * this.mCountupPct.GetOutVal()) | 0;
                if (aHeight > 0) {
                    aHeight = Math.max(aHeight, 10) | 0;
                    g.DrawImageBox(
                        aBarImages[aPointTypeIdx_2],
                        aCurX + 10,
                        1005 - aHeight,
                        aSectionWidth - 20,
                        aHeight,
                ...
                g.DrawStringCentered(
                    aPointSection_2 == (this.mPointsBreakdown.length | 0) - 1
                        ? "Last"
                        : String.format("x{0}", aPointSection_2 + 1),
                    aCurX + ((aSectionWidth / 2) | 0),
                    1043
                );
            }
            aCurX += aSectionWidth;
