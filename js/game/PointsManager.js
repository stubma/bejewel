Game.PointsManager = function Game_PointsManager() {
    this.mPointsList = [];
    this.Add_gCycleColors = Array.Create(
        6,
        6,
        new GameFramework.gfx.Color(255, 0, 0, 255),
        new GameFramework.gfx.Color(255, 255, 0, 255),
        new GameFramework.gfx.Color(0, 255, 0, 255),
        new GameFramework.gfx.Color(0, 255, 255, 255),
        new GameFramework.gfx.Color(0, 0, 255, 255),
        new GameFramework.gfx.Color(255, 0, 255, 255)
    );
...
Game.PointsManager.prototype = {
    ...
    Add: function Game_PointsManager$Add(
        theX,
        theY,
        thePoints,
        theColor,
        ...
        var aMultiplier =
            Game.BejApp.mBejApp.mBoard.mPointMultiplier * Game.BejApp.mBejApp.mBoard.GetModePointMultiplier();
        ...
        if (theX >= 0 && theY >= 0) {
            ...
            if (aNewPoints == null && Game.Resources["FONT_FLOATERS"] != null) {
                var aMode = -1;
                aNewPoints = new Game.Points(
                    Game.BejApp.mBejApp,
                    Game.Resources["FONT_FLOATERS"],
                    aString,
                    theX,
                    theY,
                    aLife,
            ...
            for (i = 0; i < 6; i++) {
                for (var aLayer = 0; aLayer < 3; aLayer++) {
                    ...
                    aNewPoints.mColorCycle[aLayer].mCycleColors.push(
                        GameFramework.gfx.Color.RGBAToInt(
                            (this.Add_gCycleColors[i].mRed * aColorPct +
                                Math.min(255, aColor.mRed * aLightness * (1.0 - aColorPct))) |
                                0,
                            (this.Add_gCycleColors[i].mGreen * aColorPct +
                                Math.min(255, aColor.mGreen * aLightness * (1.0 - aColorPct))) |
                                0,
                            (this.Add_gCycleColors[i].mBlue * aColorPct +
                                Math.min(255, aColor.mBlue * aLightness * (1.0 - aColorPct))) |
                                0,
                            (anAlpha * 255) | 0
                        )
                    );
