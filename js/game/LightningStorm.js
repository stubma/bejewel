...
Game.LightningStorm = function Game_LightningStorm(theBoard, thePiece, theType) {
    ...
    if (this.mStormType != Game.LightningStorm.EStormType.HYPERCUBE) {
        for (
            var anOffset = this.mStormType == Game.LightningStorm.EStormType.FLAMING ? -1 : 0;
            anOffset <= (this.mStormType == Game.LightningStorm.EStormType.FLAMING ? 1 : 0);
            anOffset++
        ) {
            var aRow = this.mBoard.GetRowAt(
                (thePiece.mY | 0) + ((Game.Board.GEM_HEIGHT / 2) | 0) + anOffset * Game.Board.GEM_HEIGHT
            );
            if (aRow >= 0 && aRow < this.mBoard.mRowCount && this.mStormType != Game.LightningStorm.EStormType.VERT) {
                var aZap = new Game.LightningZap(
                    this.mBoard,
                    Math.max(
                        0,
                        this.mCX - this.mStormLength * Game.Board.GEM_WIDTH - ((Game.Board.GEM_WIDTH / 2) | 0)
                    ) | 0,
                    (thePiece.mY | 0) + ((Game.Board.GEM_HEIGHT / 2) | 0) + anOffset * Game.Board.GEM_HEIGHT,
                    Math.min(
                        this.mBoard.GetColX(this.mBoard.mColCount),
                        this.mCX + this.mStormLength * Game.Board.GEM_WIDTH + ((Game.Board.GEM_WIDTH / 2) | 0)
                    ) | 0,
                    (thePiece.mY | 0) + ((Game.Board.GEM_HEIGHT / 2) | 0) + anOffset * Game.Board.GEM_HEIGHT,
                    Game.DM.gElectColors[(this.mMatchType | 0) + 1],
                    10.0,
                    this.mStormType == Game.LightningStorm.EStormType.FLAMING
                );
                this.mZaps.push(aZap);
            }
            var aCol = this.mBoard.GetColAt(
                (thePiece.mX | 0) + ((Game.Board.GEM_WIDTH / 2) | 0) + anOffset * Game.Board.GEM_WIDTH
            );
            if (aCol >= 0 && aCol < this.mBoard.mColCount && this.mStormType != Game.LightningStorm.EStormType.HORZ) {
                var aZap_2 = new Game.LightningZap(
                    this.mBoard,
                    (thePiece.mX | 0) + ((Game.Board.GEM_WIDTH / 2) | 0) + anOffset * Game.Board.GEM_WIDTH,
                    Math.max(
                        0,
                        this.mCY - this.mStormLength * Game.Board.GEM_HEIGHT - ((Game.Board.GEM_HEIGHT / 2) | 0)
                    ) | 0,
                    (thePiece.mX | 0) + ((Game.Board.GEM_WIDTH / 2) | 0) + anOffset * Game.Board.GEM_WIDTH,
                    Math.min(
                        this.mBoard.GetRowY(this.mBoard.mRowCount),
                        this.mCY + this.mStormLength * Game.Board.GEM_HEIGHT + ((Game.Board.GEM_HEIGHT / 2) | 0)
                    ) | 0,
                    Game.DM.gElectColors[(this.mMatchType | 0) + 1],
                    10.0,
        ...
        this.mLightingAlpha.SetCurve(
            "b;0,1,0.00885,1,####oCh;uZV###X^8.tQ<###Uqh*Kzk###QG###R~###hI###u~### $#### 2y### *####"
        );
    } else {
...
Game.LightningStorm.prototype = {
    ...
    AddLightning: function Game_LightningStorm$AddLightning(theStartX, theStartY, theEndX, theEndY) {
        ...
        for (
            var aLightningPointNum = 0;
            aLightningPointNum < Game.LightningStorm.NUM_LIGTNING_POINTS;
            aLightningPointNum++
        ) {
    ...
    UpdateLightning: function Game_LightningStorm$UpdateLightning() {
            ...
            for (var aCol = 0; aCol < 8; aCol++) {
                ...
                if (aPiece != null) {
                    ...
                    if (aPiece.mIsElectrocuting) {
                        ...
                        (aPiece.mColor == this.mColor || this.mColor == Game.DM.EGemColor.HYPERCUBE) &&
                        aPiece.GetScreenY() > -Game.Board.GEM_HEIGHT &&
                        !aPiece.IsFlagSet(Game.Piece.EFlag.DIG) &&
                        !aPiece.IsFlagSet(Game.Piece.EFlag.DETONATOR) &&
                        !aPiece.IsFlagSet(Game.Piece.EFlag.SCRAMBLE)
                    ) {
        ...
        if (aNumMatchingPieces > 0 && (this.mLightningVector.length == 0 || this.mBoard.mRand.Next() % aRandIdx == 0)) {
            ...
            if (anElectrocuter != null) {
                ...
                for (var aCheckPieceIdx = 0; aCheckPieceIdx < aNumMatchingPieces; aCheckPieceIdx++) {
                    ...
                    var aDist =
                        Math.min(
                            Math.abs(aCheckPiece.mCol - anElectrocuter.mCol),
                            Math.abs(aCheckPiece.mRow - anElectrocuter.mRow)
                        ) | 0;
                ...
                this.AddLightning(
                    (aNewPiece.mX | 0) + ((Game.Board.GEM_WIDTH / 2) | 0),
                    (aNewPiece.mY | 0) + ((Game.Board.GEM_HEIGHT / 2) | 0),
                    (anElectrocuter.mX | 0) + ((Game.Board.GEM_WIDTH / 2) | 0),
                    (anElectrocuter.mY | 0) + ((Game.Board.GEM_HEIGHT / 2) | 0)
                );
        ...
        for (var aLightningNum = 0; aLightningNum < (this.mLightningVector.length | 0); aLightningNum++) {
            ...
            if ((wantsCalm && this.mUpdateCnt % 5 == 0) || (!wantsCalm && this.mUpdateCnt % 2 == 0)) {
                ...
                var anEndX =
                    aLightning.mPoints[aLightning.mPoints.mIdxMult0 * (Game.LightningStorm.NUM_LIGTNING_POINTS - 1) + 0]
                        .x;
                var anEndY =
                    aLightning.mPoints[aLightning.mPoints.mIdxMult0 * (Game.LightningStorm.NUM_LIGTNING_POINTS - 1) + 0]
                        .y;
                for (
                    ...
                    var aCenterX =
                        aStartX * (1.0 - aDistAlong) +
                        anEndX * aDistAlong +
                        aCenterMult * (GameFramework.Utils.GetRandFloat() * 40.0 + aPullFactor * aLightning.mPullX);
                    var aCenterY =
                        aStartY * (1.0 - aDistAlong) +
                        anEndY * aDistAlong +
                        aCenterMult * (GameFramework.Utils.GetRandFloat() * 40.0 + aPullFactor * aLightning.mPullY);
                    if (wantsCalm) {
                        aCenterX =
                            aStartX * (1.0 - aDistAlong) +
                            anEndX * aDistAlong +
                            aCenterMult * (GameFramework.Utils.GetRandFloat() * 15.0 + aPullFactor * aLightning.mPullX);
                        aCenterY =
                            aStartY * (1.0 - aDistAlong) +
                            anEndY * aDistAlong +
                            aCenterMult * (GameFramework.Utils.GetRandFloat() * 15.0 + aPullFactor * aLightning.mPullY);
                    }
        ...
        if (
            !hasExplodingPieces &&
            aNumElectrocutedPieces == 0 &&
            aNumMatchingPieces == 0 &&
            this.mLightningVector.length == 0 &&
            !hadWildcardEffect
        ) {
    ...
    DrawLightning: function Game_LightningStorm$DrawLightning(g) {
        ...
        for (var aLightningNum = 0; aLightningNum < (this.mLightningVector.length | 0); aLightningNum++) {
            ...
            if (GameFramework.BaseApp.mApp.get_Is3D()) {
                ...
                for (
                    var aLightningPointNum = 0;
                    aLightningPointNum < Game.LightningStorm.NUM_LIGTNING_POINTS - 1;
                    aLightningPointNum++
                ) {
                    var aPoint = aLightning.mPoints[aLightning.mPoints.mIdxMult0 * aLightningPointNum + 0];
                    var aPointR = aLightning.mPoints[aLightning.mPoints.mIdxMult0 * aLightningPointNum + 1];
                    ...
                    var aColor = GameFramework.gfx.Color.UInt_FAToInt(
                        Game.DM.gElectColors[(this.mColor | 0) + 1],
                        aBrightness
                    );
                    if (aLightningPointNum == 0) {
                        aTriVertices[aTriVertices.mIdxMult0 * aTriCount + 0] = new GameFramework.gfx.TriVertex(
                            aPoint.x,
                            aPoint.y,
                            0.5,
                            aV,
                        ...
                        aTriVertices[aTriVertices.mIdxMult0 * aTriCount + 1] = new GameFramework.gfx.TriVertex(
                            aPointRD.x,
                            aPointRD.y,
                            1.0,
                            aVD,
                        ...
                        aTriVertices[aTriVertices.mIdxMult0 * aTriCount + 2] = new GameFramework.gfx.TriVertex(
                            aPointD.x,
                            aPointD.y,
                            0.0,
                            aVD,
                ...
                var aColor_2 = GameFramework.gfx.Color.UInt_AToInt(
                    Game.DM.gElectColors[(this.mColor | 0) + 1],
                    (aBrightness * 255.0) | 0
                );
                for (
                    var aLightningPointNum_2 = 0;
                    aLightningPointNum_2 < Game.LightningStorm.NUM_LIGTNING_POINTS - 1;
                    aLightningPointNum_2++
                ) {
